...
- 登録する試合結果の情報
  ...
  - JBSレーティング対象フラグ (true or false)
    - JBSレーティング対象にするには、ポイント数が 5 以上でなければならない
...
- イベントページの試合結果一覧とイベント内ランキングは、リアルタイム更新を維持する
  - 更新通知の購読には `observeQuery` を利用する
  - 初回取得/再同期は `eventId` のキー条件で取得し、Filter主体の全件走査を避ける
...
- 年度ランキングは、マテリアライズドビュー方式で提供する
  ...
  - イベントページのランキングは、イベント内試合結果の同一データセットから計算する
    - 試合結果一覧とランキングで別々に読み取りを行わず、単一購読結果を再利用する
...
- 内部テーブル(`EventUserContribution`)の利用目的
  ...
  - `EventUserContribution` は運用補助(監査・差分確認・バックフィル)にも利用可能とする
  - 一般ユーザー向けUIは `EventUserContribution` を直接参照しない
- 更新トリガー
...
- `MatchResult` はイベントページの取得最適化のため、`eventId` を起点に検索できるインデックスを持つ
  - 推奨: `eventId` をパーティションキー、試合日時(`matchDate`,`matchTime`)をソートキーとする
  - これにより、イベント単位の試合結果取得を Query ベースで実行する
...
1. `MatchResult` に `eventId` 起点インデックスを追加する
  - 想定インデックス: `eventId`(PK) + `matchDate`,`matchTime`(SK)
  - 目的: イベントページでの試合結果取得を DynamoDB Query に統一する
2. イベントページの取得経路をキー条件ベースに移行する
  - 現在の Filter 条件による取得を廃止し、`eventId` のキー条件で取得する
  - 取得した試合結果配列を、試合結果一覧とイベント内ランキングで共用する
3. リアルタイム更新は `observeQuery` を継続利用する
  - 初回同期: `eventId` キー条件で対象イベントの試合結果を取得
  - 差分同期: 作成/更新/削除の購読イベントを同一データセットへ反映
4. ランキング算出はイベントページ内で再計算する
  - 購読結果の配列を入力としてランキングを再計算する
  - ランキングのための追加読み取りは行わない
5. `EventUserContribution` は内部用途に限定する
...
1. ページ初期表示
  - クライアント: `eventId` をキー条件に `MatchResult` を取得する
  - AppSync Resolver: `MatchResult` の `eventId` 起点インデックスを利用
  - DynamoDB: `Query` 実行(イベント単位)
2. 初回描画
  - 取得結果を状態に保持し、試合結果一覧を描画する
  - 同じ結果セットからイベント内ランキングを算出して描画する
3. リアルタイム更新
  - `observeQuery` で `MatchResult` の変更を受信する
  - 受信した差分を状態にマージし、一覧とランキングを同時更新する
4. 再同期
  - 接続復帰や再購読時は、`eventId` キー条件で再取得する
  - 一覧とランキングの整合性は単一データセットで担保する

#### 運用確認項目

1. CloudWatch で `MatchResult` テーブルの `Query` / `Scan` 比率を監視する
2. イベントページ表示時に `Scan` が増加していないことを確認する
3. 試合結果の追加・更新・削除時に一覧とランキングが同時に更新されることを確認する
